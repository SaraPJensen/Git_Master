...
class ModelParams:
    n_shifts: int 
    n_neurons: int    #The number of remeining neurons
    output_dim: int
...
# class Simplical(nn.Module):

#     def __init__(self, params):
#         super().__init__()

#         self.input_dim = params.n_neurons
#         self.output_dim = params.output_dim

#         self.GCN_layer = GCNConv(params.n_neurons, params.output_dim)


#     def forward(self, x, edge_index):
#         return self.GCN_layer(x, edge_index)
...
class OuterModel(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        edge_indices, sparse = dense_to_sparse(connectivity) 
  
        #print("Connectivity shape: ", connectivity.shape)
        #print("Edge index shape: ", edge_indices.shape)

        simplices = self.simplex1(connectivity, edge_indices)
        simplices = simplices.relu()
        simplices = self.simplex2(simplices, edge_indices)

...
def fit(
    ...
    for epoch in range(1, epochs+1):
        ...
        val_loss = val(model, val_loader, criterion, device, dataset_params.output_dim)
        log.info(
...
def fit_2d(
    ...
    loss_folder = model.makedirs(dataset_params.network_type)
    file = open(f"{loss_folder}/loss.csv", "w")
    file.write("epoch,train_loss,val_loss,val_2s_loss,val_3s_loss\n")
    file.close()
    ...
    for epoch in range(1, epochs+1):
        train_loss = train(model, train_loader, optimizer, criterion, device)
        val_loss, val_2s_loss, val_3s_loss = val(model, val_loader, criterion, device, dataset_params.output_dim)
        log.info(
            f"{epoch}) train : {train_loss:.4f}, val : {val_loss:.4f}, val_2s : {val_2s_loss:.4f}, val_3s : {val_3s_loss:.4f}")

        file = open(f"{loss_folder}/loss.csv", "a")
        file.write(f"{epoch},{train_loss},{val_loss},{val_2s_loss},{val_3s_loss}\n")
        file.close()

...
class sequenceDataset(data.Dataset):
    ...
    def __init__(self, num_categories, seq_len, size= 1000):
        super().__init__()
        self.num_categories = num_categories
        self.seq_len = seq_len
        #self.size = size
        
        self.data = torch.from_numpy(np.array([np.sort(np.random.choice(np.arange(1,num_categories), seq_len, replace=False, p = (1/(num_categories-1))*np.ones(num_categories-1) )) for i in np.arange(size)]))
...
pretrained_filename = os.path.join(CHECKPOINT_PATH, 'priyas_saved_model_upd_dataset_final.pth')
if os.path.isfile(pretrained_filename):

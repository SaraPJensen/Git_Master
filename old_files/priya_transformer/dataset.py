...
'''
Questions 
* What should be returned as the class labels?
* How to deal with negative numbers there?
* Why does the model still make good predictions when I change the dataset without retraining?
'''


class smallDataset(data.Dataset):
    def __init__(self, seq_len, sample_size = 2000):
        super().__init__()
        ...
        self.num_categories = self.max_start * 2**(seq_len-1) * 2
        ...
        for i in range (sample_size):
            start1 = np.random.randint(0, self.max_start)
            start2 = np.random.randint(0, self.max_start)

            for j in range(0, seq_len - 1, 2):
                array[i, j] = start1
                array[i, j+1] = start2
        ...
        for i in range (sample_size):
            ...
            for j in range(seq_len):
                array[i, j] = np.random.randint(0, 500)
    ...
    def __getitem__(self, idx):
        ...
        #This is wrong, should return the rolled array

        #What should be returned here??  
        #Problem: cannot have negative values
